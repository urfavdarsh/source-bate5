...
async def checkAdded(_: Client, response: ChatMemberUpdated):
    ...
    img = await app.download_media(bot.photo.big_file_id, file_name=os.path.join("./", "bot.jpg")) if bot.photo else 'https://telegra.ph/file/42e3d373a734919936a96.jpg'
    await _.send_photo(
        chat_id = response.chat.id, 
        photo = img,
        caption = caption,
